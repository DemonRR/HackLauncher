...
HackLauncher（渗透武器库）用于集中管理、检索、检查和启动本地安全工具。当前版本为 **1.1.7**，采用 **Go + Wails v2** 构建，可管理应用程序、命令、Python、Java、文件、文件夹和 URL，并提供工具体检、运行审计、配置备份与系统托盘等能力。

## 更新日志

### v1.1.7

- 修复程序隐藏后任务栏或系统托盘偶发无响应、无法恢复主窗口的问题：托盘改为在主 OS 线程注册，并与 Wails 共用 Windows 消息循环。
- 增加托盘菜单点击和重复启动触发单实例唤醒的运行日志，便于后续排查窗口恢复问题。
- 配置备份调整为最多保留最新三份，程序启动时自动清理超额历史备份。

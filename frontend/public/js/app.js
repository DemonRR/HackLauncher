        ...
        try {
          ...
          reader.onload = async (event) => {
            try {
              ...
              // 在覆盖当前内存配置之前完成结构校验；后端还会进行第二次校验。
              const isObject = value => value && typeof value === 'object' && !Array.isArray(value);
              if (!isObject(newConfig) ||
                  !Array.isArray(newConfig.items) ||
                  !Array.isArray(newConfig.categories) ||
                  (newConfig.settings !== undefined && !isObject(newConfig.settings)) ||
                  (newConfig.environment !== undefined && !isObject(newConfig.environment))) {
                throw new Error('无效的配置文件');
              }
              const migration = normalizeImportedConfig(newConfig);
              ...
              document.getElementById('confirm-ok-btn').onclick = async () => {
                ...
                try {
                  ...
                  document.getElementById('confirm-modal').classList.add('hidden');
                  showNotification('成功', migration.corrections > 0
                    ? `配置已导入，并自动修正 ${migration.corrections} 个兼容字段`
                    : '配置已导入', 'success');
                } catch (error) {
...
async function saveSettings() {
  try {
    ...
    const requestedHotkey = document.getElementById('show-window-hotkey').value.trim() || 'Ctrl+Shift+H';
    const registeredHotkey = await window.api.updateShowWindowHotkey(requestedHotkey);
    ...
    showNotification('错误', '保存设置失败: ' + message, 'error');
  }
}
...
function hotkeyFromKeyboardEvent(event) {
  ...
  else if (/^F(?:[1-9]|1[0-9]|2[0-4])$/.test(event.key.toUpperCase())) key = event.key.toUpperCase();

  if (!key) return { error: '仅支持 A-Z、0-9 和 F1-F24 作为主按键' };
  if (!modifiers.length) return { error: '请至少同时按下 Ctrl、Alt、Shift 或 Win' };
  return { value: [...modifiers, key].join('+') };
}

function setupShowWindowHotkeyInput() {
  const input = document.getElementById('show-window-hotkey');
  const resetButton = document.getElementById('reset-show-window-hotkey');
  const help = document.getElementById('show-window-hotkey-help');
  if (!input || !resetButton || !help) return;

  input.addEventListener('keydown', event => {
    event.preventDefault();
    event.stopPropagation();
    if (['Control', 'Alt', 'Shift', 'Meta'].includes(event.key)) {
      help.innerHTML = '<i class="fas fa-keyboard"></i> 继续按下一个字母、数字或功能键';
      help.classList.remove('error');
      return;
    }
    const result = hotkeyFromKeyboardEvent(event);
    if (result.error) {
      help.innerHTML = `<i class="fas fa-exclamation-circle"></i> ${result.error}`;
      help.classList.add('error');
      return;
    }
    input.value = result.value;
    help.innerHTML = '<i class="fas fa-check-circle"></i> 新快捷键将在保存设置后生效';
    help.classList.remove('error');
  });

  input.addEventListener('focus', () => {
    help.innerHTML = '<i class="fas fa-keyboard"></i> 请按下新的组合键';
    help.classList.remove('error');
  });
  resetButton.addEventListener('click', () => {
    input.value = 'Ctrl+Shift+H';
    help.innerHTML = '<i class="fas fa-undo"></i> 已恢复默认，保存设置后生效';
    help.classList.remove('error');
  });
}

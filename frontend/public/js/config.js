...
// Normalize legacy Electron exports and remove fields that do not belong to an item's type.
function normalizeImportedConfig(config) {
  let corrections = 0;
  if (!config || typeof config !== 'object' || Array.isArray(config)) return { config, corrections };
  ...
  config.environment = config.environment && typeof config.environment === 'object' ? config.environment : {};
  config.usageStats = config.usageStats && typeof config.usageStats === 'object' ? config.usageStats : {};
  config.sortOrders = config.sortOrders && typeof config.sortOrders === 'object' ? config.sortOrders : { all: [], favorites: [] };
  config.categoryOrder = Array.isArray(config.categoryOrder) ? config.categoryOrder : [];

  const typeAliases = {
    app: 'application', exe: 'application', application: 'application',
    cmd: 'command', shell: 'command', command: 'command',
    py: 'python', python: 'python', jar: 'java', java: 'java',
  ...
  config.items = config.items.filter(item => item && typeof item === 'object').map(item => {
    ...
    let type = typeAliases[originalType.toLowerCase()] || typeAliases[originalType] || originalType;
    const command = String(item.command || '').trim();
    const pathOnly = command.replace(/^['"]|['"]$/g, '').toLowerCase();
    if (type === 'application' && pathOnly.endsWith('.jar')) type = 'java';
    if (type === 'application' && pathOnly.endsWith('.py')) type = 'python';
    if (!['command', 'application', 'python', 'java', 'file', 'folder', 'url'].includes(type)) {
      type = pathOnly.endsWith('.jar') ? 'java' : pathOnly.endsWith('.py') ? 'python' : pathOnly.endsWith('.exe') ? 'application' : 'file';
    }
    if (item.type !== type) {
      item.type = type;
      corrections++;
    ...
    if (typeof item.launchParams !== 'string') {
    ...
    if (type === 'java') {
      item.javaEnvironmentId = item.javaEnvironmentId == null ? '' : String(item.javaEnvironmentId);
      item.javaProgramParams = typeof item.javaProgramParams === 'string' ? item.javaProgramParams : '';
    } else {
      for (const field of ['javaEnvironmentId', 'javaProgramParams']) {
        if (field in item) {
          delete item[field];
          corrections++;
        }
    ...
    for (const field of ['arguments', 'pythonArgs', 'javaArgs']) {
      if (field in item) {
        delete item[field];
        corrections++;
      }
  ...
  config.environment.javaEnvironments = Array.isArray(config.environment.javaEnvironments)
    ? config.environment.javaEnvironments.filter(env => env && typeof env === 'object').map(env => ({
        ...env,
        id: env.id == null ? '' : String(env.id),
        name: String(env.name || ''),
        path: String(env.path || '')
      }))
    : [];
  if (!Array.isArray(config.environment.customPaths)) config.environment.customPaths = [];
  config.environment.defaultJavaEnvironmentId = config.environment.defaultJavaEnvironmentId == null
    ? ''
    : String(config.environment.defaultJavaEnvironmentId);
...
async function loadConfig() {
  try {
    ...
    if (config) {
      const migration = normalizeImportedConfig(config);
